...
class MyHospitalDB:
    ...
    def addDoctor(self, docTableName, docCred):
        ...
        try:
            [name, spe, age, addr, cont, fees, mSalary] = docCred  # unpacking list of creds so that i can insert all in table
            cursor, conn = getCursor()
            query = f"SELECT * FROM {docTableName}" # doing to get all the columns name of table
            cursor.execute(query)
            docDet = cursor.fetchall()
            column_names = [col[0] for col in cursor.description]  # In order to work this, you have to execute above two line.
    ...
    def addDetails(self, doc="", nurse="", patient="", others=""):  # given default arg so that i can send from where this fun is getting a single arg according to the doc or nurse or other. and below in this function bodu i can verify what arg got passed in this function.
        if doc == "doctor":
    ...
    def deleteDetails(self, doc="", nurse="", patient="", others=""):
        if doc == "doctors" :
            ...
            self.doctorDetails(docTableName)
            print("Enter the Doctor's name of which you want to remove : ", end=" ") # id and username both are unique so we can anyone.username seems more intuitive
            d = input()
